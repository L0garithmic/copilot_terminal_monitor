...
export function activate(context: vscode.ExtensionContext) {
  const activeExecutions = new Map<vscode.TerminalShellExecution, ExecutionData>();
  ...
  const createStatusBar = () => {
    ...
    const alignment = config.get<string>("statusBarAlignment") === "Left" ? vscode.StatusBarAlignment.Left : vscode.StatusBarAlignment.Right;
    ...
    if (config.get<boolean>("enabled") && config.get<boolean>("statusBarAlwaysVisible")) {
      statusBarItem.show();
    }
  };
  ...
  const startFlashing = (color: string) => {
    ...
    flashInterval = setInterval(async () => {
      ...
      await vscode.workspace.getConfiguration("workbench").update("colorCustomizations", customizations, vscode.ConfigurationTarget.Global);
    }, 500);
  };
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("terminal-idle-monitor.openSettings", () => {
      const panel = vscode.window.createWebviewPanel("terminalIdleMonitorSettings", "Copilot Terminal Monitor Settings", vscode.ViewColumn.One, { enableScripts: true });
      panel.webview.html = getSettingsHtml(vscode.workspace.getConfiguration("terminalIdleMonitor"));
      panel.webview.onDidReceiveMessage(async (message) => {
        if (message.command === "update") {
          await vscode.workspace.getConfiguration("terminalIdleMonitor").update(message.key, message.value, vscode.ConfigurationTarget.Global);
          panel.webview.html = getSettingsHtml(vscode.workspace.getConfiguration("terminalIdleMonitor"));
  ...
  const checkInterval = setInterval(async () => {
    ...
    if (activeData) {
      ...
      const cmdSummary = activeData.commandLine.length > 30 ? activeData.commandLine.substring(0, 27) + "..." : activeData.commandLine;

      if (!isNotificationShowing && !activeData.idleNotified && idle >= (config.get<number>("idleTimeout") || 60)) {
        activeData.idleNotified = true;
        isNotificationShowing = true;
        const isObnoxious = config.get<boolean>("obnoxiousMode");
        if (isObnoxious) { startFlashing(config.get<string>("obnoxiousColor") || "#ff0000"); }
        vscode.window.showWarningMessage(
          `IDLE: "${cmdSummary}" (${idle}s)`, 
          { modal: isObnoxious }, 
          "Reset Timer", "Snooze 5m", "Snooze 10m", "Snooze 15m"
        ).then(async s => {
          isNotificationShowing = false;
          if (isObnoxious) { await stopFlashing(); }
          if (s === "Reset Timer") { 
      ...
      if (!isNotificationShowing && !activeData.totalNotified && elapsed >= (config.get<number>("totalTimeout") || 300)) {
        ...
        vscode.window.showInformationMessage(
          ...
          { modal: isObnoxious }, 
          "Snooze 5m", "Snooze 10m", "Snooze 15m"
        ).then(async s => {
          isNotificationShowing = false;
          if (isObnoxious) { await stopFlashing(); }
      ...
      if (config.get<boolean>("statusBarAlwaysVisible")) {
        statusBarItem.text = config.get<boolean>("displayIdleText") ? "$(terminal-cmd) Idle" : "$(terminal-cmd)";
        statusBarItem.show();
      } else {
        statusBarItem.hide();
      }
  ...
  context.subscriptions.push(vscode.window.onDidStartTerminalShellExecution(async (event) => {
    const data: ExecutionData = { startTime: Date.now(), lastActivity: Date.now(), terminal: event.terminal, commandLine: event.execution.commandLine.value || "Unknown", idleNotified: false, totalNotified: false, snoozeUntil: 0 };
    activeExecutions.set(event.execution, data);
    try {
      for await (const _ of event.execution.read()) { data.lastActivity = Date.now(); data.idleNotified = false; }
    } catch {} finally { activeExecutions.delete(event.execution); }
  }));
  context.subscriptions.push(vscode.window.onDidEndTerminalShellExecution(e => activeExecutions.delete(e.execution)));
}
